...
- @rdccs 讨论了将规则描述系统与判题后端结合的工作，以及其中的跨进程通信问题；
...
在原型设计中，我采用了 Puppeteer 库 @puppeteer2023 。该库可用于控制 Chromium 浏览器的行为，如加载页面、审查元素等等。Puppeteer 库提供了一个 Node.js 的 API，因此可以直接在 Node.js 环境中使用。稍后 @rdccs 所呈现的控制系统也在 Node.js 环境中运行，因此 Web 段的判题控制会相对容易很多。

另一方面，我们还需要将 Web 程序呈现在网页上。我们假定目前判题后端接收到的是一系列静态网页素材，那么浏览器可以有两种方式加载这些素材：
- 通过 HTTP 服务器加载，这是最常见的方式，也是原型设计采用的方式。
- 通过本地文件系统协议（`file://`）加载，这种方式可以避免 HTTP 服务配置，拥有更优的性能。但是本地文件系统协议的跨域访问通常是被禁止的 @cors2023 ，从而许多功能无法正常运行。

#fix_id

为了尽可能在生产环境模拟 Web 程序的运作，我们选择本地启动一个轻量的 HTTP 服务器。在原型设计中，我们使用了 Fastify 库 @fastify2023。它也是 Node.js 环境下的新型 HTTP 服务端库，相比传统的 Express 等库，它的性能更好。
...
原型设计并没有选择直接调用 UIA，而是选择其上的一个包装库 FlaUI  @flaui2022。该库为 C\# 库，可以简单地用 .NET 运行时来测试所有 Windows 窗体程序。在原型设计中，我们使用了 .NET 7 运行时；虽然它不在操作系统上预装，但是能够获得更多的性能提升，并提升开发人员的编码效率。
...
`turtle` 库的设计来源于 Logo 编程语言的绘图库。@turtle2023 该绘图库假想一只带着画笔的海龟可以接受简单的命令，例如向前走 100 步，或者左转 30 度。通过对这只海龟发送命令，可以让它绘制出较为复杂的图形，例如正方形，三角形，圆等。@Goldman2004 Python 的 `turtle` 库提供若干全局函数，以实现类似 Logo 绘图的命令式接口；同时，它也提供面向对象的接口，以实现更为复杂的图形。@turtle2023
...
非 Windows 的 GUI 程序则更加难以处理。非 Windows 系统上，GUI 程序的实现多种多样：既可以直接使用 Wayland 或 X @wayland2018；也可以使用 GTK、Qt 等跨平台的 GUI 库。对于这些程序，需要针对性地设计判题后端，没有很好的统一处理的途径。因此，本次原型设计对非 Windows 的 GUI 程序暂时不做考虑。
...
考察非传统的 OJ，如基于自定义规则的 AutoLab，则使用了规约式的设计方法：要求教师或比赛组织者提供一个程序（或脚本），对于给定的用户程序，输出其得分。@autolab2023 这种方式的优点是灵活性高，可以适应各种各样的题目类型，但是缺点也很明显：需要教师或比赛组织者极高的编程能力，且必须对 GUI 程序十分熟悉，以给出各种细节的判定规则。
...
充分利用已有的库，是软件设计的一大原则，即“不重复造轮子”。@Kemper2005 因此我们首先考察基于已有编程语言设计规则描述的方式是否可行。
...
现在考虑 RDCCS 已经拿到一个用户程序和一个判题规则（经规则描述系统处理后）。那么，RDCCS 需要执行判题规则所指定的行为，并将其中涉及后端操纵对象的代码转换为对后端的通信操作。这一过程需要讨论如何与判题后端进行有效、可靠的通信。
...
JSON-RPC 是基于 JSON 的一个远程过程调用（Remote Procedure Call, RPC）协议。它的基本思想是，将函数调用转换为 JSON 对象，以 HTTP 请求、TCP 请求或管道的形式发送给服务端，服务端执行相应的操作，并将结果以 JSON 对象的形式返回给客户端。JSON-RPC 在 C++、Python、C\# 语言有第三方库提供包装，为我们的编程工作提供便利。@jsonrpc2013
...
原型中提供的通信数据包括：开始判题时的判题规则和用户程序、判题过程中的判题结果和判题结束得到的判题结果。前者是由前端主动发起的，后两者则是由 RDCCS 主动发起的，因此该通信的最佳实践应当为双端主动通信；信道选择上应为全双工的。在 HTTP 及更高层次的通信协议中，WebSocket 最为适合。@websockets2023 原型设计中出于项目复杂度的考虑，没有使用 WebSocket，而是直接使用基于简单 HTTP 的客户端轮询；相关的优劣讨论将在 @frontend 中讨论。
...
相比 Python 等编程语言，作为规则描述底层的 JavaScript 语言的标准库可谓十分匮乏。例如，出于历史原因，JavaScript 甚至没有提供合适、完备的字符串类型与数型数据相互转换的方法。而 @transpiler 强调过作为规则描述的首要目的是减少规则编写者的学习成本，因此在设计规则描述系统时，需要引入必要的额外机制来补充语言的缺陷。

在 GUI-OJ 软件系统中，我选择了 _运行时库_（Runtime Library）的概念来解决这一问题。运行时库是一组函数的集合，它们可以被规则描述者直接调用，以完成一些常见的操作。例如，GUI-OJ 中的运行时库包含了字符串与数型数据的相互转换、简单的系统功能调用、图像比对等功能。运行时库的实现是由规则描述系统提供的，因此规则描述者可以直接调用运行时库中的函数，而无需关心其实现细节。
...
运行时库是为了补足规则描述系统采用的 DSL 的不足而设计的，因此它被设计为嵌入规则描述系统。
...
DefDef 语言使用 `deftools` 转译器读取。该转译器会生成每个模块所需的数据，如 @fig_structure_defdef 所示。原型设计中，DefDef 会对规则描述系统和前端生成相关数据，包括从代码形式到 JSON-RPC 请求的转换规则、智能提示（类型）信息和可视化编辑信息等。
...
如 @backend-frontend 所讨论，前端将与 RDCCS 交互，原型实现采用 HTTP 通信。故在原型设计中，前端使用简单的 Web 程序实现。
...
+ *用户代码提交处*：为于页面右侧，提供用户代码编辑器或上传用户代码；
+ *判题结果*：位于页面下方，显示判题结果。
...
GUI-OJ 的设计目标包含让规则的制订更加轻松易用。因此，我为此设计了可视化编辑器的模块，提供基于拖拽、点击等交互方式的规则编辑功能，如 @fig_frontend_blockly 所示。
...
用户程序可能通过设计恶意谬构的代码，利用编译器本身的缺陷，导致编译器栈溢出或无法正常响应 @gccbug2020
。最简单的攻击形式，即通过预处理指令引入 `/dev/random` 设备，从而导致随机数生成器快速竭尽，消耗系统资源。此外，用户程序也可能通过引入 `/etc/passwd` 等系统文件，读取关键系统配置以准备其它破坏手段。因此，编译程序也应有上述安全性保障，如在 `seccomp` 下运行编译器或使用第三方云编译方案等。

为减少应对编译安全的额外开发成本，我建议优先考虑可执行文件的提交方式；仅针对解释型编程语言（如原型所述 Web 程序和 Python 程序）启用源代码提交。因此，下文不再讨论编译过程的安全性，仅考虑用户程序运行时的安全保障。

== GUI-OJ 系统的安全性保障措施考察

原型设计缺乏安全性考虑。原型设计没有对用户程序的运行时间、内存或 CPU 用量做限制。也没有限制系统调用、用户权限或虚拟化技术。虽然作为原型演示，安全不是首要考虑的因素；但是作为完整系统而言，安全性保障是必不可少的。

首先，_鸵鸟策略_ 不失为一种选择。所谓鸵鸟策略，即遇到系统问题后，直接重启系统的消极解决办法。事实上，在理想的部署方案（见 @deployment）下，鸵鸟策略是可行的。但是该部署方案对硬件的需求较高，不具有实践意义。因此我们仍然从常规的方面考虑安全问题。

首先，对于时空限制等客观因素，可以设计一个专用的外部程序，监控用户进程的运行时间、内存或 CPU 用量，在达到临界值时予以终止。该程序的设计本身并不复杂，但是这涉及到更多的 RDCCS 跨进程通信问题——判题后端与监控程序的并发问题也需要得到妥善解决。

其次，原型设计对用户程序的可访问系统调用没有做限制。在这一方面，GUI-OJ 与传统 OJ 的解决方法不具有共通性。传统 OJ 可以使用 `seccomp` 白名单，以限制绝大多数越权尝试；因为传统 OJ 所执行的 C/C++ 程序非常接近操作系统底层，合法程序对操作系统调用的数量是及其有限的。但是图形用户界面程序天然需要调用各种窗口系统的接口甚至底层绘图设备的接口，这使得白名单限制是不现实的。另一方面，如 @categories 讨论，本文所述的图形界面程序全部在 Windows 上运行，而 Windows 没有类似 `seccomp` 的机制。但可考虑第三方软件如 Sandboxie @sandboxie2023，并使用黑名单方式限制用户程序的系统调用。

再次，我们考察用户权限系统上的安全性措施。在 Windows 上，这似乎是可行的：我们可以利用 Windows 用户权限系统与 NTFS 文件权限系统共同限制用户程序的操作。但类似地，跨用户的操作增加了 RDCCS 的设计复杂度。

最后，我们可以考虑使用虚拟化技术如 Windows 沙盒、虚拟机工作站等。但是，跨虚拟机的进程控制是非常困难的，这要求我们对 RDCCS 做更谨慎的设计——此部分将同系统部署于 @deployment 讨论。
...
为了支持大量的用户判题请求，判题后端通常为若干容器构成的集群，它们可以由云计算服务商如 Amazon、阿里云等提供；AutoLab 所使用的部署方案即是如此。@autolab2014 我们可以根据这一模型提出类似的 GUI-OJ 系统的部署方案。

== GUI-OJ 系统的部署方案设计

综合 @security 提出的安全性需求，以及开发难度上的考量，我们可以类似地将判题后端以容器集群的形式部署在本地物理机或 ECS 云主机上，如 @fig_structure_deploy 所示。这样的部署方案可以满足安全性需求，也易于实现和应用。
...
其中，用户程序在启动时被发送给某个空闲判题后端，同时开启该判题后端的 HTTP 接口的监听。随后，发起判题的 RDCCS 服务器向该后端发送 JSON-RPC 请求，这之后的过程和 @ipc 所讨论的本地方式就别无二异了。
