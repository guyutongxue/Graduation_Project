...
非 Windows 的 GUI 程序则更加难以处理。非 Windows 系统上，GUI 程序的实现多种多样：既可以直接使用 Wayland 或 X #citation；也可以使用 GTK、Qt 等跨平台的 GUI 库。对于这些程序，需要针对性地设计判题后端，没有很好的统一处理的途径。因此，本次原型设计对非 Windows 的 GUI 程序暂时不做考虑。
...
原型前端整体基于 React 框架。React 是一个响应式 JavaScript 框架，以函数式思想提供各层组件的数据、状态和渲染逻辑。#citation 原型前端的主要界面如 @fig_frontend_screenshot 所示。
...
Google 主导的 Blockly 项目提供可视化编程的框架，该项目被广泛应用于 Scratch、App Inventor 等面向初学者的可视化编程语言中。#citation Blockly 项目使用 JSON 定义工具栏、块等可视化编辑元素，使用 XML 作为可视化编辑器的内部表示。原型使用 DefDef 导出 Blockly 所需的工具栏、块定义，并提供了相应的导出 JavaScript 的转换函数。当用户选择使用可视化编辑器时，前端会通过上述定义将编辑器的形式转换为规则 DSL，然后发送给规则描述系统进行进一步处理。

== 完整的 GUI-OJ 系统前端概述

参考现有的 OJ 系统 #citation，完整的 GUI-OJ 系统应当类似地包含如下功能：
...
为了防止上述行为，OJ 系统通常会采取一些安全措施 #citation。根据防御措施的作用层级和复杂度，可采取：
...
用户程序可能通过设计恶意谬构的代码，利用编译器本身的缺陷，导致编译器栈溢出或无法正常响应 #citation
...
其次，原型设计对用户程序的可访问系统调用没有做限制。在这一方面，GUI-OJ 与传统 OJ 的解决方法不具有共通性。传统 OJ 可以使用 `seccomp` 白名单，以限制绝大多数越权尝试；因为传统 OJ 所执行的 C/C++ 程序非常接近操作系统底层，合法程序对操作系统调用的数量是及其有限的。但是图形用户界面程序天然需要调用各种窗口系统的接口甚至底层绘图设备的接口，这使得白名单限制是不现实的。另一方面，如 @categories 讨论，本文所述的图形界面程序全部在 Windows 上运行，而 Windows 没有类似 `seccomp` 的机制。但可考虑第三方软件如 Sandboxie #citation,
